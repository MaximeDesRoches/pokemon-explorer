...
export function getJSON(url:string, data: Record<string, string> = {}, type = METHOD_GET, headers = []) {
...
export function pokemonApi(endpoint:string) {
	const url = `https://pokeapi.co/api/v2/${endpoint}`;
	return getJSON(url, {}, METHOD_GET);
}